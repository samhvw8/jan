...
# RAG Plugin targets
build-plugin:
	cd tauri-plugin-rag && cargo build

test-plugin:
	cd tauri-plugin-rag && cargo test

check-plugin:
	cd tauri-plugin-rag && cargo check

clean-plugin:
	cd tauri-plugin-rag && cargo clean

# Build plugin examples
build-plugin-examples:
	cd tauri-plugin-rag && cargo build --examples

# Run plugin examples
run-plugin-example-basic:
	cd tauri-plugin-rag && cargo run --example basic_usage

run-plugin-example-full:
	cd tauri-plugin-rag && cargo run --example full_usage
...
ifeq ($(OS),Windows_NT)
	...
	-powershell -Command "Remove-Item -Recurse -Force ./tauri-plugin-rag/target"
